...
function parseDates(str) {
  ...
  const defaultDate = new Date(curYear, curMonth, curDate);

  const DaysOfWeek = {'Monday': 1, 'Tuesday': 2, 'Wednesday': 3, 'Thursday': 4, 'Friday': 5, 'Saturday': 6, 'Sunday': 7};

  //check for 'Today' string
  if (str.includes('Today')) return defaultDate;

  //check for day of week
  const daysArr = Object.keys(DaysOfWeek);
  let timeLapse;

  daysArr.forEach(day => {
    if (str.includes(day)) {
      timeLapse = curDay - DaysOfWeek[day];
    }
  })
  if (timeLapse < 0) {
    return new Date(curYear, curMonth, curDate - timeLapse - 7)
  }
  if (timeLapse === 0) {
    let newDate = curDate - 7;
    return new Date(curYear, curMonth, newDate)
  }
  if (timeLapse > 0) {
    return new Date(curYear, curMonth, curDate - timeLapse)
  }
