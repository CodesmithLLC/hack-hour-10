...
function getPINs(observed) {
  ...
  let observedNum = Number(observed);
  let ans = {};
  let ansArr = [];

  ansArr.push(String(observedNum));
  if (observedNum === 8) ansArr.push(String(0));

  for (let j = 0; j < layOut.length; j++) {
    for (let k = 0; k < layOut[j].length; k++) {
      if (layOut[j][k] === observedNum - 3) ansArr.push(String(layOut[j][k]));
      if (layOut[j][k] === observedNum + 3) ansArr.push(String(layOut[j][k]));
      if (layOut[j][k] === observedNum - 1) ansArr.push(String(layOut[j][k]));
      if (layOut[j][k] === observedNum + 1) ansArr.push(String(layOut[j][k]));
    }
  }
  ans[observedNum] = ansArr;
  return ans;
}
