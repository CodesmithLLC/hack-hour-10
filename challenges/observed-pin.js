...
function getPINs(observed, index = 0) {
  const adj = {
    0: ['8', '0'],
    1: ['1', '2', '4'],
    2: ['1', '2', '3', '5'],
  ...
  if (index === observed.length - 1) return adj[observed[index]];
  const theRest = getPINs(observed, index + 1);
  return adj[observed[index]].reduce((acc, num) => [...acc, ...theRest.map(seq => num.concat(seq))], []);
}
