...
function determinePosition(digit) {
  const keypad = [
    [1, 2, 3],
    [4, 5, 6],
    [7, 8, 9],
...
function determineNumber(position) {
  const keypad = [
    [1, 2, 3],
    [4, 5, 6],
    [7, 8, 9],
  ...
  return keypad[position[1] - 1][position[0] -1];
}

function determinePositions(position) {
  const positions = [position];
  for (let i = 0; i < 4; ++i) {
    if (i === 0) {
      const pos = position[1] - 1;
      if(pos >= 1) {
        positions.push([position[0], pos]);
      }
    }
    if (i === 1) {
      const pos = position[1] + 1;
      if(pos <= 3 || pos === 3 && position[0] === 2) {
        positions.push([position[0], pos]);
      }
    }
    if (i === 2) {
...
console.log(determinePosition(0));
console.log(determinePositions([2,4]));
console.log(determineNumber([2,4]));
